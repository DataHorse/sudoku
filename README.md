...
1. Create a new repository on GitHub (e.g. `inkwell-sudoku`) — public repos get free Pages hosting.
2. Upload **all files in this folder**, keeping the same structure:
   ...
   `git commit -m "Inkwell Sudoku"`, then push to the GitHub repo.)
3. In the repo, go to **Settings → Pages**.
4. Under **Build and deployment**, set **Source** to "Deploy from a branch", branch = `main`,
   folder = `/ (root)`. Save.
5. GitHub gives you a URL like `https://YOUR-USERNAME.github.io/inkwell-sudoku/`. It takes
...
4. Tap **Add**. An "Inkwell" icon appears on the home screen and opens full-screen, no browser bar.
...
### Desktop (Chrome/Edge)
Click the install icon (⊕) in the address bar, or the ⋮ menu → **Install Inkwell Sudoku**.
...
`inkwell-sudoku-v2`) so old caches are cleared out.
...
- `css/style.css` — the "Inkwell" design system (see `PROMPT.md` for the design brief).
